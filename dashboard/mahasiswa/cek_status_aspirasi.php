...
if ($schemaReady && $kode !== '') {
    ...
    if (!$stmt) {
        die('Prepare cek status aspirasi gagal: ' . mysqli_error($conn));
    }
...
<div class="page-light">
    ...
    <section class="status-layout">
        <form class="status-card" method="POST" action="cek_status_aspirasi.php">
            ...
            <input 
                ...
                name="kode_aspirasi" 
                value="<?= h($kode); ?>" 
                placeholder="Contoh: ASP-260602101530-42" 
                required
            >
        ...
        <?php if ($schemaReady && $kode !== '' && !$data) { ?>
            <div class="alert danger">
                Data aspirasi dengan kode tersebut tidak ditemukan.
            </div>
        <?php } ?>
            ...
            <article class="detail-card">
                ...
                <div class="meta-grid">
                    ...
                    <div>
                        <span>Pengirim</span>
                        <strong>
                            <?= ((int) $data['is_anonim'] === 1) ? 'Anonim' : h($data['nama_mahasiswa'] ?: 'Mahasiswa'); ?>
                        </strong>
                    </div>
                        ...
                        <?php while ($row = mysqli_fetch_assoc($komentar)) { ?>
                            <div class="comment-item <?= ($row['level_user'] ?? '') === 'admin' ? 'admin-comment' : ''; ?>">
                                <div class="comment-head">
                                    <strong>
                                        <?= ($row['level_user'] ?? '') === 'admin' ? 'Pengurus/Admin' : 'Mahasiswa'; ?>
                                    </strong>
                ...
                <a class="btn-heading compact" href="detail_aspirasi.php?id=<?= (int) $data['id_aspirasi']; ?>">
                    Lihat Detail
                </a>
            </article>
        <?php } ?>
