<?php
session_start();
// Sesuaikan path ke folder config
require_once '../config/database.php';

// Jika admin sudah login sebelumnya, langsung alihkan ke dashboard admin
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if (!empty($username) && !empty($password)) {
            ...
            if ($admin && password_verify($password, $admin['password'])) {
                ...
                if (empty($admin['id_akses']) || $admin['status_verifikasi'] !== 'Terverifikasi') {
                    $error = "Akun Anda belum diverifikasi oleh Superadmin atau belum memiliki ID Akses.";
                } else {
                    $_SESSION['pending_admin_data'] = $admin;
                    header("Location: login_admin.php?step=verifikasi"); 
                    exit;
                }
            } else {
    ...
    // ----------- TAHAP 2: PROSES VERIFIKASI ID AKSES -----------
    if (isset($_POST['verifikasi_id_akses'])) {
        $input_id_akses = trim($_POST['id_akses']);

        if (!empty($input_id_akses)) {
            if (isset($_SESSION['pending_admin_data'])) {
                $admin_sukses = $_SESSION['pending_admin_data'];

                // Mencocokkan data session dengan input (mengabaikan besar/kecil huruf)
                if (strcasecmp($admin_sukses['id_akses'], $input_id_akses) === 0) {
                    
                    // SET SESSION ADMIN SESUAI PERMINTAAN session_check.php
                    $_SESSION['is_logged_in'] = true;
                    $_SESSION['is_admin'] = true;
                    $_SESSION['peran']   = 'admin';                 
                    $_SESSION['user_id'] = $admin_sukses['id_admin'];   
                    $_SESSION['nama']    = $admin_sukses['nama_lengkap'];  
                    ...
                    $error = "ID Akses Verifikasi Admin Salah!";
                }
            } else {
            ...
            <div class="md:w-[58%] px-12 py-10 flex flex-col justify-center bg-white">
                ...
                <?php if ($step == 'login'): ?>
                    <div class="text-center mb-8">
                        <h2 class="text-[28px] font-bold text-[#1F3D68]">Login Administrator</h2>
                        <p class="text-gray-500 text-sm mt-1 font-semibold">Silahkan login untuk mengelola sistem</p>
                    </div>

                    <form action="login_admin.php?step=login" method="POST" class="space-y-4">
                        <div class="relative">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <i class="fa-regular fa-user text-gray-500"></i>
                            </div>
                            <input type="text" name="username" required placeholder="Username" class="w-full pl-11 pr-4 py-2.5 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-[#1F3D68] text-sm font-semibold text-gray-700 placeholder-gray-500 transition">
                        </div>
                        ...
                        <button type="submit" name="login_admin" class="w-full bg-[#1F3D68] hover:bg-[#152e52] text-white font-bold py-3 px-4 rounded-lg focus:outline-none text-xs transition duration-200 mt-4">
                            LOGIN ADMIN
                        </button>
                    </form>
                ...
                <?php elseif ($step == 'verifikasi'): ?>
                    ...
                    <div class="text-center mb-8">
                        <h2 class="text-[28px] font-bold text-[#1F3D68]">Verifikasi Admin</h2>
                        <p class="text-gray-500 text-xs mt-1 font-semibold leading-relaxed">
                            Masukkan kode <b>ID AKSES</b> Anda sebagai<br>
                            Administrator sistem.
                        </p>
                    </div>

                    <form action="login_admin.php?step=verifikasi" method="POST" class="space-y-6">
                        <div class="relative w-4/5 mx-auto">
                            <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                <i class="fa-solid fa-key text-gray-500"></i>
                            </div>
                            <input type="text" name="id_akses" required placeholder="ID AKSES ADMIN" autocomplete="off" class="w-full pl-11 pr-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-[#1F3D68] text-sm font-bold text-gray-700 placeholder-gray-500 transition tracking-widest text-center uppercase">
                        </div>
                    ...
                    <div class="text-center mt-6">
                        <a href="login_admin.php?step=login" class="text-[12px] text-gray-500 hover:text-red-500 hover:underline transition font-bold"><i class="fa-solid fa-arrow-left"></i> Kembali ke Login Utama</a>
                    </div>
